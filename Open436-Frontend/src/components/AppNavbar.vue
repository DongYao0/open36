...
let refreshTimer
function refreshAssignments() { if (auth.token && document.visibilityState === 'visible') assignmentStore.refresh() }
function onVisibilityChange() { if (document.visibilityState === 'visible') assignmentStore.refresh(true) }
onMounted(() => {
  document.addEventListener('click', closeDropdowns)
  document.addEventListener('visibilitychange', onVisibilityChange)
  refreshAssignments()
  refreshTimer = window.setInterval(refreshAssignments, 60000)
})
onUnmounted(() => {
...
watch(() => auth.token, (token) => token ? assignmentStore.refresh(true) : assignmentStore.clear())
</script>
...
.avatar-wrap{position:relative;display:inline-flex}.assignment-badge{position:absolute;top:-7px;right:-9px;display:flex;align-items:center;justify-content:center;min-width:19px;height:19px;padding:0 5px;border:2px solid #21175d;border-radius:999px;background:#ff3b45;color:#fff;font-size:10px;font-weight:800;line-height:1}.assignment-entry{color:#ffb8c2}
