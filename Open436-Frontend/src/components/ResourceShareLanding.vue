      ...
      <div v-if="posts.length" class="resource-grid">
        <button v-for="(post, index) in posts" :key="post.id" class="resource-card" @click="$emit('open', post)">
          ...
          <div class="resource-copy">
            ...
            <p>{{ post.preview }}</p>
            <footer><span>{{ post.author }} · {{ post.createdAt }} · {{ post.votes }} 收藏</span><b>查看资源 ↗</b></footer>
          </div>
        </button>
      </div>
      <div v-else-if="!loading" class="resource-empty">{{ error || '还没有资源，来分享第一份吧。' }}</div>
      <div v-if="loading" class="resource-loading">正在加载资源…</div>
      <div v-if="totalCount > 0" class="resource-pagination">
        <label>
          <span>每页展示</span>
          <select :value="pageSize" :disabled="loading" @change="$emit('page-size-change', Number($event.target.value))">
            <option v-for="size in PAGE_SIZES" :key="size" :value="size">{{ size }} 条</option>
          </select>
        </label>
        <span class="page-summary">共 {{ totalCount }} 条 · 第 {{ currentPage }} / {{ totalPages }} 页</span>
        <Pagination :model-value="currentPage" :total-pages="totalPages" @update:model-value="$emit('page-change', $event)" />
      </div>
    </section>
  </section>
</template>
...
const props = defineProps({ posts: { type: Array, default: () => [] }, totalCount: { type: Number, default: 0 }, currentPage: { type: Number, default: 1 }, pageSize: { type: Number, default: 15 }, loading: Boolean, error: String })
defineEmits(['open', 'page-change', 'page-size-change'])
...
.resource-stage{position:relative;min-height:100vh;width:100%;margin:0;padding:78px clamp(28px,8vw,130px) 120px;background:transparent}.resource-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:18px}.resource-card{min-height:168px;padding:21px;text-align:left;display:flex;flex-direction:column;color:#fff;border:1px solid rgba(229,237,255,.33);border-radius:12px;background:linear-gradient(130deg,rgba(53,68,122,.88),rgba(77,87,151,.77));box-shadow:0 14px 30px rgba(26,35,88,.25);backdrop-filter:blur(14px);transition:transform .25s ease,background .25s ease,border-color .25s ease}.resource-card:hover{border-color:rgba(246,238,255,.76);background:linear-gradient(130deg,rgba(66,79,146,.95),rgba(112,84,175,.88));transform:translateY(-7px)}.card-type{color:#bac8ff;font-size:11px;font-weight:800;letter-spacing:.08em}.resource-card strong{margin:12px 0;font-size:18px;line-height:1.4}.resource-card footer{display:flex;justify-content:space-between;gap:8px;margin-top:auto;padding-top:13px;color:#d8dfff;font-size:11px;border-top:1px solid rgba(255,255,255,.16)}.resource-card footer span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.resource-empty,.resource-loading{padding:70px 24px;text-align:center;color:rgba(255,255,255,.86)}
.resource-pagination{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:18px;max-width:1160px;margin:28px auto 0;padding:13px 18px;border:1px solid rgba(246,230,188,.3);border-radius:16px;background:rgba(12,17,57,.78);backdrop-filter:blur(14px)}.resource-pagination label{display:flex;align-items:center;gap:9px;color:#cbd5f5;font-size:12px}.resource-pagination select{padding:8px 10px;border:1px solid rgba(246,230,188,.32);border-radius:9px;background:#151c4b;color:#fff7e7;outline:none}.page-summary{color:#aebae0;font-size:12px;font-variant-numeric:tabular-nums}.resource-pagination :deep(.pagination){justify-self:end;padding:0}.resource-pagination :deep(.page-btn){border-color:rgba(246,230,188,.28);background:#171d4b;color:#d8dfff}.resource-pagination :deep(.page-btn.active){border-color:#f7c86b;background:linear-gradient(135deg,#e97846,#88427b);color:#fff}
