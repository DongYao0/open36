-- =====================================================
-- Open436 论坛服务 - 数据库迁移脚本
-- 版本: V2
-- 创建日期: 2026-08-01
-- 说明: 创建论坛内容/评论相关表
--       (content/comment 模型均为 managed=False，Django 迁移不建表，
--        需由此脚本按 db-init 约定管理)
-- =====================================================

-- 1. posts 帖子表（content.Post）
...
-- 2. post_edit_history 编辑历史表（content.PostEditHistory）
...
CREATE INDEX IF NOT EXISTS idx_post_edit_history_created_at ON public.post_edit_history(created_at DESC);

-- 3. replies 回复表（comment.Reply）
...
-- 4. post_likes 帖子点赞表（comment.PostLike）
...
-- 5. post_favorites 帖子收藏表（comment.PostFavorite）
...
-- 6. reply_likes 回复点赞表（comment.ReplyLike）
...
-- 7. share_records 分享记录表（comment.ShareRecord）
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_topics_name ON public.topics(name);

-- 9. topic_follows 话题关注表（comment.TopicFollow）
...
-- 10. user_follows 用户关注表（comment.UserFollow）
